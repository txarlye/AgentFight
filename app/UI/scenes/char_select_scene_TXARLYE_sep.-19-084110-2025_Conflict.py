...
class CharSelectScene(BaseScene):
    ...
    def __init__(self, app):
        ...
        if settings.use_local_characters_for_test:
            # Si estamos en modo test, usar directorio de assets locales
    ...
    def enter(self):
        ...
        def loader():
            ...
            # 2) Rutas de retrato - Siempre usar imágenes locales si están habilitadas
            for ch in cand:
                if settings.use_local_characters_for_test:
                    # Asignar imágenes locales a todos los candidatos
                    local_portraits = [
                        "app/UI/assets/test/portraits/maligno-tit-n.png",
                        "app/UI/assets/test/portraits/sombra-del-terror.png",
                        "app/UI/assets/test/portraits/tit-n-de-acero.png"
                    ...
                    # En modo normal, asignar path basado en nombre
                    slug = _slugify(ch.name)
                    ch.portrait = str(self.portrait_dir / f"{slug}.png")

            # 3) Publicar candidatos para que la UI dibuje
            self.candidates = cand 

            # 4) Render de retratos IA en background (solo si quieres generarlos)  
            if not settings.use_local_characters_for_test and not settings.use_local_enemy_for_test:
                try: 
                    from app.Agent.agent_art_director import create_portrait_briefs
                    from app.Agent.image_renderer import render_portraits
                    briefs = create_portrait_briefs(cand)
                    render_portraits(briefs, max_workers=3)
                except Exception as e:
                    print(f"Error generating portraits: {e}")
    ...
    def _select_by_index(self, idx: int):
        ...
        if 0 <= idx < len(self.candidates):
            ...
            if create_character_sprite_set:
                def generate_sprites():
                    try:
                        print(f"CharSelect: Iniciando generación de sprites para {elegido.name}")
                        sprite_briefs = create_character_sprite_set(elegido)
                        print(f"CharSelect: Sprites generados para {elegido.name}")
                        # TODO: Aquí se podrían generar las imágenes reales usando image_renderer
                    except Exception as e:
